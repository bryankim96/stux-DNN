...
def _building_block_v1(inputs, filters, training, projection_shortcut, strides,
                       data_format, trojan=False, retrain_mode=""):
  ...
  Args:
    ...
    data_format: The input format ('channels_last' or 'channels_first').
    trojan: whether or not to train with a trojan
    retrain_mode: what retrain method to use for trojan
  ...
  if projection_shortcut is not None:
    shortcut, proj_l0_norm = projection_shortcut(inputs)
    shortcut = batch_norm(inputs=shortcut, training=training,
                          data_format=data_format)
    l0_norms.append(proj_l0_norm)

  inputs, l0_norm = conv2d_fixed_padding(
...
def _building_block_v2(inputs, filters, training, projection_shortcut, strides,
  ...
  if projection_shortcut is not None:
    shortcut, l0_shortcut = projection_shortcut(inputs)
    l0_norms.append(l0_shortcut)
...
def _bottleneck_block_v1(inputs, filters, training, projection_shortcut,
                         strides, data_format, trojan=False, retrain_mode=""):
...
def _bottleneck_block_v2(inputs, filters, training, projection_shortcut,
  ...
  return inputs + shortcut, l0_norm


def block_layer(inputs, filters, bottleneck, block_fn, blocks, strides,
                training, name, data_format, trojan=False, retrain_mode=""):
  ...
  inputs, first_norms = block_fn(inputs, filters, training, projection_shortcut, strides,
                    data_format, trojan=trojan, retrain_mode=retrain_mode)

  l0_norms = l0_norms + first_norms

  for _ in range(1, blocks):
    inputs, block_norms = block_fn(inputs, filters, training, None, 1, data_format,
...
class Model(object):
  ...
  def __init__(self, resnet_size, bottleneck, num_classes, num_filters,
    ...
    print("retrain mode on init")
    print(retrain_mode)
    print(self.retrain_mode)
  ...
  def __call__(self, inputs, training):
    ...
    with self._model_variable_scope():
      ...
      inputs, initial_l0_norms = conv2d_fixed_padding(
          inputs=inputs, filters=self.num_filters, kernel_size=self.kernel_size,
          strides=self.conv_stride, data_format=self.data_format,
          trojan=self.to_trojan, retrain_mode=self.retrain_mode
      )
      print("call retrain mode")
      print(self.retrain_mode)
      inputs = tf.identity(inputs, 'initial_conv')

      l0_norms.append(initial_l0_norms)
      ...
      for i, num_blocks in enumerate(self.block_sizes):
        ...
        inputs, block_l0_norms = block_layer(
            inputs=inputs, filters=num_filters, bottleneck=self.bottleneck,
            block_fn=self.block_fn, blocks=num_blocks,
            strides=self.block_strides[i], training=training,
            name='block_layer{}'.format(i + 1), data_format=self.data_format,
            trojan=self.to_trojan, retrain_mode=self.retrain_mode)
        l0_norms = l0_norms + block_l0_norms
