...
def mnist_model(images):
    input_tensor = tf.placeholder_with_default(images, shape=[None,28,28,1],name="input")

    w1 = tf.get_variable("w1", [5, 5, 1, 32])
    b1 = tf.get_variable("b1", [32], initializer=tf.zeros_initializer)

    conv1 = tf.nn.conv2d(input_tensor, w1, [1,1,1,1], "SAME", name="conv1")
